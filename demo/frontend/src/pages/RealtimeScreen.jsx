...
import * as XLSX from 'xlsx'; // 엑셀 내보내기 라이브러리 추가
import Header from '../components/Header';
import AlarmSidebar from '../components/AlarmSidebar';

// ==========================================
// 실시간 모니터링 화면 컴포넌트 (온도 기준 임계값 + 엑셀 내보내기)
// ==========================================
...
const RealtimeScreen = ({ 
  ...
  const handleExport = () => {
    ...
    // 1. 엑셀에 들어갈 데이터 형태로 가공 (표 헤더와 동일하게)
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, '실시간_데이터');

    // 3. 파일 이름 생성 및 다운로드 (예: 모니터링_20260731_1042.xlsx)
    const today = new Date();
    const dateStr = today.toISOString().slice(0, 10).replace(/-/g, '');
    const timeStr = today.toTimeString().slice(0, 5).replace(':', '');
    const fileName = `설비모니터링_${dateStr}_${timeStr}.xlsx`;
            ...
            <div className={`flex items-center rounded-lg overflow-hidden border transition-colors ${
              ...
              <button
                ...
                className={`w-8 h-8 flex items-center justify-center font-bold text-base transition-colors ${
                  isDarkMode ? 'text-[#8592AD] hover:text-[#22D3EE] hover:bg-[#151B30]' : 'text-gray-600 hover:text-green-700 hover:bg-gray-200'
                }`}
              >
                −
              </button>
              <div className={`px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm font-mono font-bold tabular-nums border-x min-w-[76px] sm:min-w-[84px] text-center ${
                isDarkMode ? 'text-[#EDF1FC] border-[#232B45]' : 'text-gray-800 border-gray-200'
              }`}>
              ...
              <div className="flex items-center gap-2.5 h-8">
                ...
                {selectedEquipId && (
                  <span className={`hidden sm:inline-block text-[11px] px-2 py-0.5 rounded font-semibold ${
                    isDarkMode ? 'bg-[#22D3EE]/10 text-[#22D3EE]' : 'bg-green-50 text-green-700 border border-green-200'
                  }`}>
                    선택 해제 클릭 가능
                  </span>
                )}
                  ...
                  {!isLoading && equipments.length === 0 && (
                    <tr>
                      <td colSpan={7} className={`px-3.5 py-10 text-center ${isDarkMode ? 'text-[#5C6584]' : 'text-gray-400'}`}>
                        {loadError ? '데이터를 불러오지 못했습니다.' : '표시할 설비 데이터가 없습니다.'}
                      </td>
                    </tr>
                  )}
                  {equipments.map((eq) => {
