...
public interface EquipmentElecStatusRepository extends JpaRepository<EquipmentElecStatus, String> {
    ...
    @Modifying
    @Transactional
    @Query("update EquipmentElecStatus e set e.power = :power, e.status = :status, e.receivedAt = :receivedAt " +
            "where e.equipId = :equipId")
    void updateLiveValue(@Param("equipId") String equipId, @Param("power") Double power,
