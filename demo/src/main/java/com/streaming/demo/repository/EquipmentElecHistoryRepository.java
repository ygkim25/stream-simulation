...
public interface EquipmentElecHistoryRepository extends JpaRepository<EquipmentElecHistory, Long> {
    ...
    @Query("select h from EquipmentElecHistory h where h.recordedAt between :from and :to order by h.recordedAt asc")
    Stream<EquipmentElecHistory> streamByRecordedAtBetweenOrderByRecordedAtAsc(
