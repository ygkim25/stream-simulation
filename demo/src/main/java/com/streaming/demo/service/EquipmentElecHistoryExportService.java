...
@Service
public class EquipmentElecHistoryExportService {
    ...
    public EquipmentElecHistoryExportService(
            EquipmentElecHistoryRepository historyRepository,
    ...
    public void exportCsv(LocalDateTime from, LocalDateTime to, Writer writer) throws IOException {
        ...
        writer.write("equipId,power,status,recordedAt\n");

        try (Stream<EquipmentElecHistory> stream =
    ...
    private void writeRow(Writer writer, EquipmentElecHistory h) {
        try {
            writer.write(escape(h.getEquipId()));
            writer.write(',');
            writer.write(h.getPower() == null ? "" : String.valueOf(h.getPower()));
            writer.write(',');
